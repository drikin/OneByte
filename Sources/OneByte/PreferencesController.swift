...
struct PreferencesPanel: View {
    ...
    private let statusFont = Font.system(.caption, design: .monospaced)
        ...
        Task {
            ...
            do {
                ...
                guard let httpResp = resp as? HTTPURLResponse, httpResp.statusCode == 200 else {
                    ...
                    await MainActor.run { statusText = "エラー: HTTP \(code)"; statusColor = .red; testing = false }
                    return
                }
                if let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                   ...
                   let msg = first["message"] as? [String: Any], let content = msg["content"] as? String {
                    await MainActor.run {
                        statusText = "✅ 接続OK: \(content.trimmingCharacters(in: .whitespacesAndNewlines))"
                        statusColor = .green; testing = false
                    }
                }
            } catch {
                await MainActor.run {
                    statusText = "❌ エラー: \(error.localizedDescription)"
                    statusColor = .red; testing = false
                }
            }
        }
    }
